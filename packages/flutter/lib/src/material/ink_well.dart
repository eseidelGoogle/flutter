...
class _InkResponseState<T extends InkResponse> extends State<T> {
  ...
  void updateHighlight(bool value) {
    ...
    if (value) {
      if (_lastHighlight == null) {
        ...
        _lastHighlight = Material.of(context).highlightAt(
            ...
            shape: config.highlightShape, onRemoved: () {
    ...
    if (config.onHighlightChanged != null) config
        .onHighlightChanged(value != null);
  }
  ...
  void _handleTapDown(Point position) {
    ...
    splash = Material.of(context).splashAt(
        ...
        containedInWell: config.containedInWell, onRemoved: () {
      if (_splashes != null) {
  ...
  void _handleDoubleTap() {
    _currentSplash?.confirm();
    _currentSplash = null;
    if (config.onDoubleTap != null) config.onDoubleTap();
  }

  void _handleLongPress() {
    _currentSplash?.confirm();
    _currentSplash = null;
    if (config.onLongPress != null) config.onLongPress();
  }
  ...
  void dependenciesChanged(Type affectedWidgetType) {
    if (affectedWidgetType == Theme &&
        _lastHighlight != null) _lastHighlight.color =
        Theme.of(context).highlightColor;
  }

  Widget build(BuildContext context) {
    final bool enabled = config.onTap != null ||
        config.onDoubleTap != null ||
