...
abstract class RenderToggleable extends RenderConstrainedBox {
  RenderToggleable(
    ...
    _tap =
        new TapGestureRecognizer(router: FlutterBinding.instance.pointerRouter)
    ...
    _reaction = new ValuePerformance<double>(
        variable: new AnimatedValue<double>(minRadialReactionRadius,
            end: kRadialReactionRadius, curve: Curves.ease),
  ...
  void set value(bool value) {
    ...
    _position
        .play(value ? AnimationDirection.forward : AnimationDirection.reverse);
  }
  ...
  void _handlePositionStateChanged(PerformanceStatus status) {
    if (isInteractive) {
      if (status == PerformanceStatus.completed && !_value) onChanged(true);
      else if (status == PerformanceStatus.dismissed &&
          _value) onChanged(false);
    }
  }

  void _handleTapDown(Point globalPosition) {
    if (isInteractive) _reaction.forward();
  }
  ...
  void _handleTapUp(Point globalPosition) {
    if (isInteractive) _reaction.reverse();
  }
