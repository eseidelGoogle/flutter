...
class _ScaffoldLayout extends MultiChildLayoutDelegate {
  void performLayout(Size size, BoxConstraints constraints) {
    ...
    final BoxConstraints toolBarConstraints =
        looseConstraints.tightenWidth(size.width);
    ...
    if (isChild(_Child.body)) {
      ...
      final BoxConstraints bodyConstraints =
          toolBarConstraints.tightenHeight(bodyHeight);
    ...
    if (isChild(_Child.bottomSheet)) {
      ...
      positionChild(
          _Child.bottomSheet,
          new Point((size.width - bottomSheetSize.width) / 2.0,
              size.height - bottomSheetSize.height));
    }

    if (isChild(_Child.snackBar)) {
      snackBarSize = layoutChild(_Child.snackBar, fullWidthConstraints);
      positionChild(
          _Child.snackBar, new Point(0.0, size.height - snackBarSize.height));
    }

    if (isChild(_Child.floatingActionButton)) {
      final Size fabSize =
          layoutChild(_Child.floatingActionButton, looseConstraints);
      final double fabX =
          size.width - fabSize.width - _kFloatingActionButtonMargin;
      double fabY = size.height - fabSize.height - _kFloatingActionButtonMargin;
      if (snackBarSize.height > 0.0) fabY = math.min(
          fabY,
          size.height -
              snackBarSize.height -
              fabSize.height -
              _kFloatingActionButtonMargin);
      if (bottomSheetSize.height > 0.0) fabY = math.min(
          fabY, size.height - bottomSheetSize.height - fabSize.height / 2.0);
...
class Scaffold extends StatefulComponent {
  ...
  static ScaffoldState of(BuildContext context) =>
      context.ancestorStateOfType(ScaffoldState);
...
class ScaffoldState extends State<Scaffold> {
  ...
  Queue<ScaffoldFeatureController<SnackBar>> _snackBars =
      new Queue<ScaffoldFeatureController<SnackBar>>();
  ...
  void _handleSnackBarStatusChange(PerformanceStatus status) {
    switch (status) {
      case PerformanceStatus.dismissed:
        ...
        if (_snackBars.isNotEmpty) _snackBarPerformance.forward();
        break;
  ...
  ScaffoldFeatureController showBottomSheet(WidgetBuilder builder) {
    ...
    bottomSheet = new _PersistentBottomSheet(
        key: bottomSheetKey, performance: performance, onClosing: () {
      assert(_currentBottomSheet._widget == bottomSheet);
      entry.remove();
    }, onDismissed: () {
    ...
    setState(() {
      _currentBottomSheet = new ScaffoldFeatureController._(
          bottomSheet, completer, () => entry.remove(), setState);
    });
  ...
  void _addIfNonNull(List<LayoutId> children, Widget child, Object childId) {
    if (child != null) children.add(new LayoutId(child: child, id: childId));
  }
  ...
  Widget get _modifiedToolBar {
    ...
    if (left == null) {
      if (config.drawer != null) {
        left = new IconButton(icon: 'navigation/menu', onPressed: openDrawer);
      } else {
    ...
    return toolBar.copyWith(padding: padding, left: left);
  }

  Widget build(BuildContext context) {
    final Widget materialBody =
        config.body != null ? new Material(child: config.body) : null;

    if (_snackBars.length > 0) {
      ModalRoute route = ModalRoute.of(context);
      if (route == null || route.isCurrent) {
        if (_snackBarPerformance.isCompleted &&
            _snackBarTimer == null) _snackBarTimer =
            new Timer(_snackBars.first._widget.duration, _hideSnackBar);
      } else {
        _snackBarTimer?.cancel();
        _snackBarTimer = null;
      }
    ...
    if (_currentBottomSheet != null ||
      ...
      if (_dismissedBottomSheets != null &&
          _dismissedBottomSheets.isNotEmpty) bottomSheets
          .addAll(_dismissedBottomSheets);
      if (_currentBottomSheet != null) bottomSheets
          .add(_currentBottomSheet._widget);
      Widget stack = new Stack(bottomSheets,
          alignment:
              const FractionalOffset(0.5, 1.0) // bottom-aligned, centered
          );
      _addIfNonNull(children, stack, _Child.bottomSheet);
    }

    if (_snackBars.isNotEmpty) _addIfNonNull(
        children, _snackBars.first._widget, _Child.snackBar);

    _addIfNonNull(
        children, config.floatingActionButton, _Child.floatingActionButton);

    if (config.drawer != null) {
      children.add(new LayoutId(
          id: _Child.drawer,
          child: new DrawerController(key: _drawerKey, child: config.drawer)));
    }
...
class _PersistentBottomSheetState extends State<_PersistentBottomSheet> {
  ...
  void _handleStatusChange(PerformanceStatus status) {
    if (status == PerformanceStatus.dismissed &&
        config.onDismissed != null) config.onDismissed();
  }
  ...
  Widget build(BuildContext context) {
    return new AlignTransition(
        ...
        child: new BottomSheet(
            ...
            builder: (BuildContext context) => new SizeObserver(
                child: config.builder(context),
                onSizeChanged: _updateChildHeight)));
  }
}
