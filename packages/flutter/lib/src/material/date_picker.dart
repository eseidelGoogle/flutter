...
class DatePicker extends StatefulComponent {
  DatePicker(
      {this.selectedDate, this.onChanged, this.firstDate, this.lastDate}) {
...
class _DatePickerState extends State<DatePicker> {
  ...
  void _handleYearChanged(DateTime dateTime) {
    ...
    if (config.onChanged != null) config.onChanged(dateTime);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return new Column(
        <Widget>[header, new Container(height: _calendarHeight, child: picker)],
  ...
  Widget build(BuildContext context) {
    ...
    switch (theme.primaryColorBrightness) {
      case ThemeBrightness.light:
        dayColor =
            mode == _DatePickerMode.day ? Colors.black87 : Colors.black54;
        yearColor =
            mode == _DatePickerMode.year ? Colors.black87 : Colors.black54;
        break;
      case ThemeBrightness.dark:
        dayColor = mode == _DatePickerMode.day ? Colors.white : Colors.white70;
        yearColor =
            mode == _DatePickerMode.year ? Colors.white : Colors.white70;
        break;
    }
    TextStyle dayStyle = headerTheme.display3
        .copyWith(color: dayColor, height: 1.0, fontSize: 100.0);
    TextStyle monthStyle =
        headerTheme.headline.copyWith(color: dayColor, height: 1.0);
    TextStyle yearStyle =
        headerTheme.headline.copyWith(color: yearColor, height: 1.0);
    ...
    return new Container(
        ...
        child: new Column(<Widget>[
          new GestureDetector(
              ...
              child: new Text(
                  new DateFormat("MMM").format(selectedDate).toUpperCase(),
                  style: monthStyle)),
          new GestureDetector(
              onTap: () => _handleChangeMode(_DatePickerMode.day),
              child: new Text(new DateFormat("d").format(selectedDate),
                  style: dayStyle)),
          new GestureDetector(
              onTap: () => _handleChangeMode(_DatePickerMode.year),
              child: new Text(new DateFormat("yyyy").format(selectedDate),
                  style: yearStyle))
        ]));
  }
}
...
class DayPicker extends StatelessComponent {
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle headerStyle =
        theme.text.caption.copyWith(fontWeight: FontWeight.w700);
    TextStyle monthStyle =
        headerStyle.copyWith(fontSize: 14.0, height: 24.0 / 14.0);
    ...
    List<Widget> rows = <Widget>[
      new Text(new DateFormat("MMMM y").format(displayedMonth),
          style: monthStyle),
      new Flex(headers, justifyContent: FlexJustifyContent.spaceAround)
    ];
    ...
    int daysInMonth = new DateTime(year, month + 1)
        .difference(new DateTime(year, month))
        .inDays;
    ...
    for (int i = 0; i < daySlots; i++) {
      ...
      if (day < 1 || day > daysInMonth) {
        ...
        if (selectedDate.year == year &&
            ...
            selectedDate.day == day) decoration = new BoxDecoration(
            backgroundColor: theme.primarySwatch[100], shape: Shape.circle);

        // Use a different font color for the current day
        TextStyle itemStyle = dayStyle;
        if (currentDate.year == year &&
            currentDate.month == month &&
            currentDate.day == day) itemStyle =
            itemStyle.copyWith(color: theme.primaryColor);

        item = new GestureDetector(onTap: () {
    ...
    for (int w = 0; w < weeksShown; w++) {
      ...
      rows.add(new Row(labels.sublist(startIndex, startIndex + days.length)));
    }
...
class _MonthPickerState extends ScrollableWidgetListState<MonthPicker> {
  ...
  int get itemCount => (config.lastDate.year - config.firstDate.year) * 12 +
      config.lastDate.month -
      config.firstDate.month +
      1;

  List<Widget> buildItems(BuildContext context, int start, int count) {
    List<Widget> result = new List<Widget>();
    DateTime startDate = new DateTime(config.firstDate.year + start ~/ 12,
        config.firstDate.month + start % 12);
    for (int i = 0; i < count; ++i) {
      DateTime displayedMonth =
          new DateTime(startDate.year + i ~/ 12, startDate.month + i % 12);
...
class YearPicker extends ScrollableWidgetList {
  YearPicker({this.selectedDate, this.onChanged, this.firstDate, this.lastDate})
...
class _YearPickerState extends ScrollableWidgetListState<YearPicker> {
  ...
  List<Widget> buildItems(BuildContext context, int start, int count) {
    TextStyle style =
        Theme.of(context).text.body1.copyWith(color: Colors.black54);
    List<Widget> items = new List<Widget>();
    for (int i = start; i < start + count; i++) {
      int year = config.firstDate.year + i;
      String label = year.toString();
      Widget item = new InkWell(key: new Key(label), onTap: () {
        DateTime result = new DateTime(
            year, config.selectedDate.month, config.selectedDate.day);
        config.onChanged(result);
      },
          child: new Container(
              height: config.itemExtent,
              decoration: year == config.selectedDate.year
                  ? new BoxDecoration(
