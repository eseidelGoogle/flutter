...
const EdgeDims _kMenuHorizontalPadding =
    const EdgeDims.only(left: 36.0, right: 36.0);
const double _kBaselineOffsetFromBottom = 20.0;
const Border _kDropDownUnderline = const Border(
    bottom: const BorderSide(color: const Color(0xFFBDBDBD), width: 2.0));
...
class _DropDownMenu<T> extends StatusTransitionComponent {
  _DropDownMenu({Key key, _DropDownRoute<T> route})
      : route = route,
        super(key: key, performance: route.performance);
  ...
  Widget build(BuildContext context) {
    ...
    final AnimatedValue<double> menuTop = new AnimatedValue<double>(
        route.rect.top,
        end: route.rect.top - route.selectedIndex * route.rect.height,
        curve: new Interval(0.25, 0.5),
        reverseCurve: const Interval(0.0, 0.001));
    final AnimatedValue<double> menuBottom = new AnimatedValue<double>(
        route.rect.bottom,
    ...
    final RelativeRect menuRect =
        new RelativeRect.fromSize(route.rect, navigatorSize);
...
class DropDownMenuItem<T> extends StatelessComponent {
  DropDownMenuItem({Key key, this.value, this.child}) : super(key: key);
...
class DropDownButton<T> extends StatelessComponent {
  DropDownButton(
      {Key key, this.items, this.value, this.onChanged, this.elevation: 8})
      : super(key: key);
  ...
  void _showDropDown(
      BuildContext context, int selectedIndex, GlobalKey indexedStackKey) {
    final RenderBox renderBox =
        indexedStackKey.currentContext.findRenderObject();
    final Rect rect = renderBox.localToGlobal(Point.origin) & renderBox.size;
    final Completer completer = new Completer<T>();
    Navigator.push(
        context,
        new _DropDownRoute<T>(
            completer: completer,
            items: items,
  ...
  Widget build(BuildContext context) {
    GlobalKey indexedStackKey =
        new GlobalKey(debugLabel: 'DropDownButton.IndexedStack');
    int selectedIndex = 0;
    ...
    return new GestureDetector(
        child: new Container(
            ...
            child: new Row(<Widget>[
              ...
              new Container(
                  child: new Icon(
                      icon: 'navigation/arrow_drop_down', size: IconSize.s36),
                  padding: const EdgeDims.only(top: 6.0))
            ], justifyContent: FlexJustifyContent.collapse)), onTap: () {
