...
class _MaterialPageTransition extends TransitionWithChild {
  _MaterialPageTransition({Key key, PerformanceView performance, Widget child})
      : super(key: key, performance: performance, child: child);

  final AnimatedValue<Point> _position = new AnimatedValue<Point>(
  ...
  Widget buildWithChild(BuildContext context, Widget child) {
    ...
    return new Transform(
        ...
        child: new Opacity(opacity: _opacity.value, child: child));
  }
}

const Duration kMaterialPageRouteTransitionDuration =
...
class MaterialPageRoute<T> extends PageRoute<T> {
  ...
  Widget buildPage(BuildContext context, PerformanceView performance,
    ...
    assert(() {
      if (result == null) debugPrint(
          'The builder for route \'${settings.name}\' returned null. Route builders must never return null.');
      assert(result != null &&
  ...
  Widget buildTransitions(BuildContext context, PerformanceView performance,
    ...
    return new _MaterialPageTransition(performance: performance, child: child);
  }
