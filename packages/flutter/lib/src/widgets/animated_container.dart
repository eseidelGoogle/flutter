...
class AnimatedBoxConstraintsValue extends AnimatedValue<BoxConstraints> {
  AnimatedBoxConstraintsValue(BoxConstraints begin,
      {BoxConstraints end, Curve curve, Curve reverseCurve})
...
class AnimatedBoxDecorationValue extends AnimatedValue<BoxDecoration> {
  AnimatedBoxDecorationValue(BoxDecoration begin,
      {BoxDecoration end, Curve curve, Curve reverseCurve})
      : super(begin, end: end, curve: curve, reverseCurve: reverseCurve);
...
class AnimatedEdgeDimsValue extends AnimatedValue<EdgeDims> {
  AnimatedEdgeDimsValue(EdgeDims begin,
      {EdgeDims end, Curve curve, Curve reverseCurve})
      : super(begin, end: end, curve: curve, reverseCurve: reverseCurve);
...
class AnimatedMatrix4Value extends AnimatedValue<Matrix4> {
  AnimatedMatrix4Value(Matrix4 begin,
      {Matrix4 end, Curve curve, Curve reverseCurve})
      : super(begin, end: end, curve: curve, reverseCurve: reverseCurve);
...
class _AnimatedContainerState extends State<AnimatedContainer> {
  ...
  void initState() {
    ...
    _performanceController = new Performance(
        duration: config.duration, debugLabel: '${config.toStringShort()}');
    _updateCurve();
    _configAllVariables();
  }

  void didUpdateConfig(AnimatedContainer oldConfig) {
    if (config.curve != oldConfig.curve) _updateCurve();
  ...
  void _updateCurve() {
    ...
    if (config.curve != null) _performance =
        new CurvedPerformance(_performanceController, curve: config.curve);
    else _performance = _performanceController;
  ...
  void _updateVariable(Animatable variable) {
    if (variable != null) _performance.updateVariable(variable);
  }
  ...
  bool _configVariable(AnimatedValue variable, dynamic targetValue) {
    if (targetValue == variable.end) return false;
  ...
  bool _configAllVariables() {
    ...
    if (config.constraints != null) {
      ...
      if (_configVariable(_constraints, config.constraints)) needsAnimation =
          true;
    } else {
      _constraints = null;
    }
    ...
    if (config.decoration != null) {
      ...
      if (_configVariable(_decoration, config.decoration)) needsAnimation =
          true;
    } else {
      _decoration = null;
    }
    ...
    if (config.foregroundDecoration != null) {
      _foregroundDecoration ??=
          new AnimatedBoxDecorationValue(config.foregroundDecoration);
      if (_configVariable(
          _foregroundDecoration, config.foregroundDecoration)) needsAnimation =
          true;
    } else {
    ...
    if (config.margin != null) {
      ...
      if (_configVariable(_margin, config.margin)) needsAnimation = true;
    } else {
      _margin = null;
    }
    ...
    if (config.padding != null) {
      ...
      if (_configVariable(_padding, config.padding)) needsAnimation = true;
    } else {
      _padding = null;
    }
    ...
    if (config.transform != null) {
      ...
      if (_configVariable(_transform, config.transform)) needsAnimation = true;
    } else {
      _transform = null;
    }
    ...
    if (config.width != null) {
      ...
      if (_configVariable(_width, config.width)) needsAnimation = true;
    } else {
      _width = null;
    }
    ...
    if (config.height != null) {
      ...
      if (_configVariable(_height, config.height)) needsAnimation = true;
    } else {
      _height = null;
    }
  ...
  void debugFillDescription(List<String> description) {
    ...
    if (_constraints != null) description.add('has constraints');
    if (_decoration != null) description.add('has background');
    if (_foregroundDecoration != null) description.add('has foreground');
    if (_margin != null) description.add('has margin');
    if (_padding != null) description.add('has padding');
    if (_transform != null) description.add('has transform');
    if (_width != null) description.add('has width');
    if (_height != null) description.add('has height');
  }
}
