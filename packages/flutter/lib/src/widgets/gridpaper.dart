...
class _GridPaperPainter extends CustomPainter {
  const _GridPaperPainter({this.color, this.interval});
...
class GridPaper extends StatelessComponent {
  GridPaper(
      {Key key, this.color: const Color(0xFF000000), this.interval: 100.0})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return new IgnorePointer(
        child: new CustomPaint(
            painter: new _GridPaperPainter(color: color, interval: interval)));
  }
}
