...
class RenderEditableParagraph extends RenderParagraph {
  ...
  double _getIntrinsicWidth(BoxConstraints constraints) {
    ...
    return constraints
        .constrainWidth(textPainter.width + _kCursorGap + _kCursorWidth);
  }
  ...
  void performLayout() {
    ...
    Size contentSize = new Size(
        textPainter.width + _kCursorGap + _kCursorWidth, textPainter.height);
    size = constraints.constrain(contentSize);

    if (_contentSize == null || _contentSize != contentSize) {
      _contentSize = contentSize;
      if (onContentSizeChanged != null) onContentSizeChanged(_contentSize);
    }
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (hasVisualOverflow) context.pushClipRect(
        needsCompositing, offset, Point.origin & size, _paintContents);
    else _paintContents(context, offset);
  }
}
